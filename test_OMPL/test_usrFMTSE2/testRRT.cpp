...
void showPath(const ompl::geometric::PathGeometric &path0,const cv::Mat &showImg)
{
    std::list<BIpoint> Path;
    for (size_t i = 0; i < path0.getStateCount(); ++i) {
        const auto *state1 = path0.getState(i)->as<ompl::base::RealVectorStateSpace::StateType>();
        Path.push_back({state1->values[0],state1->values[1],state1->values[2]/RotationCoefficient});
    }
    std::list<BIpoint> PathShow;
    PathShow.push_back(Path.front());
    BIpoint p_old = Path.front();
    // double discreteDistance = 15.0;
    for (const BIpoint &p_now : Path)
    {
    ...
    for (const BIpoint &pathP : PathShow)
    {
        ...
        for (const BIpoint &p : objectPointSet)
        {
            ...
            cv::circle(showMap, pt1, 3, cv::Scalar(127), -1);
        }
