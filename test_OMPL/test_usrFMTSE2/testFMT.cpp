...
double plan(const cv::Mat &map,const BItask &task, int64_t &ti, int64_t &to, int64_t Nnode, double &ci) 
{
...
int main() {
    ...
    int64_t Nnode = 5000;double data_Ci=0;
    for (int32_t i = 0; i < N; i++)
    {
        cv::Mat map = loadMap(taskmap.Path);
        int64_t ti, to;double ci;
        double cost = plan(map,taskmap, ti, to,Nnode, ci);
