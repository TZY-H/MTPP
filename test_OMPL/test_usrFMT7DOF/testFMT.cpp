...
double plan(const BItask &task, int64_t &ti, int64_t &to, int64_t Nnode, double &ci) 
{
    ...
    planner->setNumSamples(Nnode);
    ss.setPlanner(planner);

    int64_t start_time;
    // bool initial_solution_found = false;
    // bool cost_solution_found = false;
    // double cost_tag = ratioCost * task.miniCost[k];
    ...
    // (void) solved;   
    if(solved)
    {ob::Cost solutionCost = ss.getProblemDefinition()->getSolutionPath()->cost(ss.getProblemDefinition()->getOptimizationObjective());
        // std::cout << "Solution cost: " << solutionCost.value() << std::endl;
            ci = solutionCost.value();

    return  solutionCost.value();}
    else
    {ci = 9999999999; return 999999999;}
}
...
int main() {
    ...
    int64_t Nnode = 30000;double data_Ci=0;
    for (int32_t i = 0; i < N; i++)
    {
        int64_t ti, to;double ci;
        double cost = plan(taskmap, ti, to,Nnode, ci);
        // if(ratioCost<cost/taskmap.miniCost)
        if(5.2<cost)
        {
            // Nnode+=2000;
            i--;
