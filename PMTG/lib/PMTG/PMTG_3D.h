...
struct status_hashCompare
{
    static std::size_t hash(const status &s)
    {
        ...
        std::size_t hz = std::hash<long>()((long)(s.z * 1e10));
        return (hx ^ (hy << 1)) ^ (hz << 1); // 组合哈希值
    }
...
public:
    ...
    void InitRootSampleStatus(std::vector<status> &Xroot, int32_t rootNum, double R) //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    {
        ...
        std::uniform_real_distribution<double> distribZ(spatialExtentMin[2], spatialExtentMax[2]);
        Node sampleNode = {0, -1, {-1, -1, -1}, {}};
        int32_t newStatusNum = 0;
        int32_t sampleNumMax = 3 * sampleN;
        while (sampleNumMax-- > 0)
        {
            status x_sam = {distribX(gen), distribY(gen), distribZ(gen)};
            if (!ValidityCheck(x_sam))
                continue;
            Neighbor_search search(NeighborSearchTree, {x_sam.x, x_sam.y, x_sam.z}, 1);
            if (search.begin() == search.end() || search.begin()->second > R)
            {
                NeighborSearchTree.insert({x_sam.x, x_sam.y, x_sam.z});
    ...
    double RealCostFun(const status &S, const status &E)
    {
        ...
        else
        {
            ...
            double dy = S.y - E.y;
            double dz = S.z - E.z;
            double realCost = sqrt(dx * dx + dy * dy + dz * dz);
            ...
            bool key = false;
            double len = realCost;
            int32_t MaximumDiscrete = len / 0.04 + 2; //$$$
            for (double denominator = 2; denominator <= MaximumDiscrete; denominator *= 2)
            {
                for (int32_t i = 1; i < denominator; i += 2)
                {
                    ...
                    double z = S.z * t + (1 - t) * E.z;
                    if (!ValidityCheck({x, y, z}))
                    {
                        edgeCostMap.insert({{S, E}, doubleMAX});
                        return doubleMAX;
                    }
    ...
    void Neighbor(const status &x, std::list<status> &Xnear, const double R)
    {
        ...
        for (const auto &point : result_points)
            Xnear.push_back({point.x(), point.y(), point.z()});
    }
    ...
    void InitializeInvironment(statusSpaceStruct &space)
    {
        ...
        frontierMin = min(min(statusSpace.boxSpace.lenx, statusSpace.boxSpace.leny), statusSpace.boxSpace.lenz);
        frontierMax = max(max(statusSpace.boxSpace.lenx, statusSpace.boxSpace.leny), statusSpace.boxSpace.lenz);
    }
    ...
    double ConnectionRadius()
    {
        ...
        return 0.6 * pow(2 * radius_1 * radius_2 * radius_3, _1_n);
    }

    double getsyncCostRadius()
    {
        return 1.3 * min(RootNodeRadius(treelist.size()), ConnectionRadius());
    }

    void InitializeSubTree(int32_t subTreeNum, int32_t BsamNum = 10000)
    {
        treeInitStruct initSubTreeStruct;
        initSubTreeStruct.EstimateCostFun = std::bind(&PMTGR3::EstimateCostFun, this,
                                                      std::placeholders::_1,
                                                      std::placeholders::_2);
        initSubTreeStruct.EdgeEstimateCostFun = std::bind(&PMTGR3::EstimateCostFun, this,
                                                          std::placeholders::_1,
                                                          std::placeholders::_2);
        initSubTreeStruct.RealCostFun = std::bind(&PMTGR3::RealCostFun, this,
                                                  std::placeholders::_1,
                                                  std::placeholders::_2);
        initSubTreeStruct.Neighbor = std::bind(&PMTGR3::Neighbor, this,
                                               std::placeholders::_1,
                                               std::placeholders::_2,
                                               std::placeholders::_3);
    ...
    double GetRGGOptPath(const status S, const status E, std::list<status> &Path, double OptCost = -1) // MT*
    {
        ...
        initSubTreeStructS.Neighbor = std::bind(&PMTGR3::Neighbor, this,
                                                std::placeholders::_1,
                                                std::placeholders::_2,
                                                std::placeholders::_3);
        initSubTreeStructS.EdgeEstimateCostFun = EdgeEstimateCostFunS;
        initSubTreeStructS.RealCostFun = std::bind(&PMTGR3::RealCostFun, this,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2);
        initSubTreeStructS.EstimateCostFun = std::bind(&PMTGR3::EstimateCostFun, this,
                                                       std::placeholders::_1,
                                                       std::placeholders::_2);
        ...
        initSubTreeStructE.Neighbor = std::bind(&PMTGR3::Neighbor, this,
                                                std::placeholders::_1,
                                                std::placeholders::_2,
                                                std::placeholders::_3);
        initSubTreeStructE.EdgeEstimateCostFun = EdgeEstimateCostFunE;
        initSubTreeStructE.RealCostFun = std::bind(&PMTGR3::RealCostFun, this,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2);
        initSubTreeStructE.EstimateCostFun = std::bind(&PMTGR3::EstimateCostFun, this,
                                                       std::placeholders::_1,
                                                       std::placeholders::_2);
