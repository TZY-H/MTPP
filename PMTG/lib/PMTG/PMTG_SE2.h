...
struct status
{
    ...
    {
        return std::abs(x - other.x) < 1e-10 && std::abs(y - other.y) < 1e-10 && std::abs(r - other.r) < 1e-10;
    }
    bool operator!=(const status &other) const
    {
        return std::abs(x - other.x) > 1e-10 || std::abs(y - other.y) > 1e-10 || std::abs(r - other.r) > 1e-10;
...
struct status_hashCompare
{
    static std::size_t hash(const status &s)
    {
        ...
        std::size_t hr = std::hash<long>()((long)(s.r * 1e10));
        return (hx ^ (hy << 1)) ^ (hr << 1); // 组合哈希值
    }
...
private:
    ...
    double RotationCoefficient = 1.0;
...
public:
    ...
    void InitRootSampleStatus(std::vector<status> &Xroot, int32_t rootNum, double R) //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    {
        ...
        std::uniform_real_distribution<double> distribR(spatialExtentMin[2], spatialExtentMax[2]);
        Node sampleNode = {0, -1, {-1, -1, -1}, {}};
        int32_t newStatusNum = 0;
        int32_t sampleNumMax = 3 * sampleN;
        while (sampleNumMax-- > 0)
        {
            status x_sam = {distribX(gen), distribY(gen), distribR(gen)};
            if (!ValidityCheck(x_sam))
                continue;
            Neighbor_search search(NeighborSearchTree, {x_sam.x, x_sam.y, RotationCoefficient * x_sam.r}, 1);
            if (search.begin() == search.end() || search.begin()->second > R)
            {
                NeighborSearchTree.insert({x_sam.x, x_sam.y, RotationCoefficient * x_sam.r});
    ...
    void BatchSampleStatus()
    {
        ...
        std::uniform_real_distribution<double> distribY(spatialExtentMin[1], spatialExtentMax[1]);
        std::uniform_real_distribution<double> distribR(spatialExtentMin[2], spatialExtentMax[2]);
        Node sampleNode = {doubleMAX, -1, {-1, -1, -1}, {}};
    ...
    {
        // objectPointSet
        double costh = cos(x.r);
        double sinth = sin(x.r);
        for (const status &p : objectPointSet)
        {
            double px = costh * p.x - sinth * p.y + x.x;
            double py = sinth * p.x + costh * p.y + x.y;
    ...
    double RealCostFun(const status &S, const status &E)
    {
        ...
        else
        {
            double dx = S.x - E.x;
            double dy = S.y - E.y;
            double dr = RotationCoefficient * (S.r - E.r);
            ...
            bool key = false;
            double len = realCost;
            int32_t MaximumDiscrete = len / 15 + 2; //$$$
            for (double denominator = 2; denominator <= MaximumDiscrete; denominator *= 2)
            {
                for (int32_t i = 1; i < denominator; i += 2)
                {
                    ...
                    double r = S.r * t + (1 - t) * E.r;
                    if (!ValidityCheck({x, y, r}))
                    {
                        edgeCostMap.insert({{S, E}, doubleMAX});
                        return doubleMAX;
                    }
    ...
    void InitializeSubTree(int32_t subTreeNum, int32_t BsamNum = 10000)
    {
        ...
        initSubTreeStruct.EstimateCostFun = std::bind(&PMTGSE2::EstimateCostFun, this,
                                                      std::placeholders::_1,
                                                      std::placeholders::_2);
        initSubTreeStruct.EdgeEstimateCostFun = std::bind(&PMTGSE2::EstimateCostFun, this,
                                                          std::placeholders::_1,
                                                          std::placeholders::_2);
        initSubTreeStruct.RealCostFun = std::bind(&PMTGSE2::RealCostFun, this,
                                                  std::placeholders::_1,
                                                  std::placeholders::_2);
        initSubTreeStruct.Neighbor = std::bind(&PMTGSE2::Neighbor, this,
                                               std::placeholders::_1,
                                               std::placeholders::_2,
                                               std::placeholders::_3);
    ...
    double GetRGGOptPath(const status S, const status E, std::list<status> &Path, double OptCost = -1) // MT*
    {
        ...
        initSubTreeStructS.Neighbor = std::bind(&PMTGSE2::Neighbor, this,
                                                std::placeholders::_1,
                                                std::placeholders::_2,
                                                std::placeholders::_3);
        initSubTreeStructS.EdgeEstimateCostFun = EdgeEstimateCostFunS;
        initSubTreeStructS.RealCostFun = std::bind(&PMTGSE2::RealCostFun, this,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2);
        initSubTreeStructS.EstimateCostFun = std::bind(&PMTGSE2::EstimateCostFun, this,
                                                       std::placeholders::_1,
                                                       std::placeholders::_2);
        ...
        initSubTreeStructE.Neighbor = std::bind(&PMTGSE2::Neighbor, this,
                                                std::placeholders::_1,
                                                std::placeholders::_2,
                                                std::placeholders::_3);
        initSubTreeStructE.EdgeEstimateCostFun = EdgeEstimateCostFunE;
        initSubTreeStructE.RealCostFun = std::bind(&PMTGSE2::RealCostFun, this,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2);
        initSubTreeStructE.EstimateCostFun = std::bind(&PMTGSE2::EstimateCostFun, this,
                                                       std::placeholders::_1,
                                                       std::placeholders::_2);
    ...
    void showPath(std::list<status> &Path, cv::Mat &showImg)
    {
        ...
        double discreteDistance = 15.0;
        for (const status &p_now : Path)
        {
            double dx = p_old.x - p_now.x;
            double dy = p_old.y - p_now.y;
            double dr = RotationCoefficient * (p_old.r - p_now.r);
            double realCost = sqrt(dx * dx + dy * dy + dr * dr);
            double len = realCost;
            ...
            for (int32_t i = 1; i <= MaximumDiscrete; i++)
            {
                ...
                double x = p_now.x * t + (1 - t) * p_old.x;
                double y = p_now.y * t + (1 - t) * p_old.y;
                double r = p_now.r * t + (1 - t) * p_old.r;
                status pp = {x, y, r};
                if (originalEstimateCostFun(pp, PathShow.back()) > 12.0)
        ...
        for (const status &pathP : PathShow)
        {
            ...
            double costh = cos(pathP.r);
            double sinth = sin(pathP.r);
            double px_old = costh * objectPointSet.back().x - sinth * objectPointSet.back().y + pathP.x;
            double py_old = sinth * objectPointSet.back().x + costh * objectPointSet.back().y + pathP.y;
            for (const status &p : objectPointSet)
            {
                double px = costh * p.x - sinth * p.y + pathP.x;
                double py = sinth * p.x + costh * p.y + pathP.y;
